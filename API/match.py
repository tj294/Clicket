...
def updateCareerStats(batting_team, bowling_team):
    for team in [batting_team, bowling_team]:
        for player in team.players:
            ...
            player.careerStats['average'] = round(average, 2) if average != 'N/A' else 'N/A'
            player.careerStats['strike_rate'] = round((player.careerStats['runs'] / player.careerStats['balls_faced']) * 100, 2) if player.careerStats['balls_faced'] > 0 else 0.00
            ...
            if player.wicketsTaken == player.careerStats['best_bowling'][0]:
                if player.runsConceded < player.careerStats['best_bowling'][1]:
                    player.careerStats['best_bowling'][0] = player.wicketsTaken
                    player.careerStats['best_bowling'][1] = player.runsConceded
            player.careerStats['economy'] = round((player.careerStats['runs_conceded'] / player.careerStats['overs_bowled']), 2) if player.careerStats['overs_bowled'] > 0 else 0.00
            player.careerStats['bowling_average'] = round((player.careerStats['runs_conceded'] / player.careerStats['wickets']), 2) if player.careerStats['wickets'] > 0 else player.careerStats['runs_conceded']
    ...
    for batter in batting_team.batters:
        if batter.onStrike:
            ...
            if len(str(batter)) >= 12:
                print(f"{batter.fname[0]+' '+batter.lname:<12}{batter.runsScored:>10}{batter.ballsFaced:>10}{batter.fours:>6}{batter.sixes:>6}{batter.strikeRate():>8.2f}")
            else:
                print(f"{str(batter):<12}{batter.runsScored:>10}{batter.ballsFaced:>10}{batter.fours:>6}{batter.sixes:>6}{batter.strikeRate():>8.2f}")
        else:
            if len(str(batter)) >= 14:
                print(f"{batter.fname[0]+' '+batter.lname:<14}{batter.runsScored:>10}{batter.ballsFaced:>10}{batter.fours:>6}{batter.sixes:>6}{batter.strikeRate():>8.2f}")
            else:
                print(f"{str(batter):<14}{batter.runsScored:>10}{batter.ballsFaced:>10}{batter.fours:>6}{batter.sixes:>6}{batter.strikeRate():>8.2f}")
    print(f"{'-'*55:<55}")
    ...
    for bowler in bowling_team.bowlers:
        if bowler.onStrike:
            ...
            if len(str(bowler)) >= 12:
                print(f"{bowler.fname[0] + ' ' +bowler.lname:<18}{bowler.oversBowled:>4}{bowler.maidens:>10}{bowler.runsConceded:>6}{bowler.wicketsTaken:>6}{bowler.economy():>8.2f}")
            else:
                print(f"{str(bowler):<18}{bowler.oversBowled:>4}{bowler.maidens:>10}{bowler.runsConceded:>6}{bowler.wicketsTaken:>6}{bowler.economy():>8.2f}")
        else:
...
def innings_roundup(batting_team, bowling_team, round_no=0, match_no=0, out=sys.stdout):
    ...
    print("="*80, file=out)
    print(f"{batting_team} {batting_team.score}/{batting_team.wickets} ({batting_team.overs}.{batting_team.ballsFaced})", file=out)
    print("="*80, file=out)
    print(f"{'Player':<25} {'How Out':<40} {'R':<5} (B)", file=out)
    print('-'*80, file=out)
    for player in batting_team.players:
        print(f"{str(player):<25} {player.howOut:<40} {player.runsScored:<5} ({player.ballsFaced})", file=out)
    print('='*80, file=out)
    ...
    for i, player in enumerate(sorted(bowling_team.players, reverse=True)):
        ...
        if player.ballsBowled == 0 and player.oversBowled == 0:
            continue
        print(f"{str(player):<25} {str(player.oversBowled)+'.'+str(player.ballsBowled):<6} {player.maidens:<4} {player.runsConceded:<4} {player.wicketsTaken:<4} {player.economy():<6.2f}", file=out)
    print('='*80, file=out)
...
def game_summary(toss, batting_team, bowling_team, round_no, match_no):
    ...
    if len(glob('../Reports/'+filename[:-4]+'*.txt')) > 0:
        filename = filename[:-4] + '_'+str(len(glob('../Reports/'+filename[:-4]+'*.txt'))+1) + '.txt'
...
def run_match(home_team, away_team, league, round_no=0, match_no=0):
    ...
    while innings <= 2:
        ...
        batting_team.batters = [new_batter(batting_team.players[0], strike=True),
