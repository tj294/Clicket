...
LEAGUE_START_TIME = datetime.datetime.now() + datetime.timedelta(days=0, hours=0, minutes=0, seconds=5)
...
TIME_BETWEEN_MATCHES = datetime.timedelta(days=0, hours=0, minutes=0, seconds=10)
...
while rounds_played < 12:
    while matches_played < 6:
        if rounds_played == 11:
            ...
            home_team = fixtures[f"Round {rounds_played+1}"][f"Match {matches_played+1}"][0]
            away_team = fixtures[f"Round {rounds_played+1}"][f"Match {matches_played+1}"][1]
        ...
        outcome = match.run_match(home_team, away_team, league=True, round_no=rounds_played+1, match_no=matches_played+1)
    ...
    NEXT_MATCH_STARTS_AT = datetime.datetime.now() + datetime.timedelta(seconds=30)
    rounds_played += 1
    matches_played = 0
